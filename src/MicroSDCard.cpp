...
static lv_fs_res_t fs_seek(lv_fs_drv_t * drv, void * file_p, uint32_t pos, lv_fs_whence_t whence) {
    ...
    if (whence == LV_FS_SEEK_SET)      ok = fp->seek(pos, SeekSet);
    else if (whence == LV_FS_SEEK_CUR) ok = fp->seek(pos, SeekCur);
    else if (whence == LV_FS_SEEK_END) ok = fp->seek(pos, SeekEnd);
...
static void * fs_dir_open(lv_fs_drv_t * drv, const char * path) {
    ...
    // V9 แนะนำใช้ lv_malloc แทน lv_mem_alloc
    File * fp = (File *) lv_malloc(sizeof(File));
    if (!fp) return NULL;

    *fp = card->open(path);
    if (!*fp || !fp->isDirectory()) {
        if (*fp) fp->close();
        lv_free(fp); // V9 ใช้ lv_free
        return NULL;
    }
    return fp;
}
...
void MicroSDCard::useLVGL() {
    // 1. Initial basic driver
    lv_fs_drv_init(&this->fs_drv);

    // 2. Set drive letter and callback
    fs_drv.letter = 'S';
    fs_drv.user_data = this;
    
    fs_drv.ready_cb = ready_cb;
